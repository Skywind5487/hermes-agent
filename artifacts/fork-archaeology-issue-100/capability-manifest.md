...
- Hunk-level behavioral coverage: **PASS**; see `behavioral-coverage-audit.md`.
...
| `capability:compression-session-boundary` | capability | 0 | 0 | PARTIAL_UPSTREAM | SPLIT | high |
...
| `capability:memory-trim-diagnostics` | capability | 1 | 0 | PARTIAL_UPSTREAM | KEEP | medium |
| `capability:memory-trim-policy` | capability | 1 | 0 | PARTIAL_UPSTREAM | PORT | high |
...
| `capability:session-search-context-hydration` | capability | 0 | 0 | PARTIAL_UPSTREAM | PORT | high |
...
| `capability:sqlite-write-contention-policy` | capability | 0 | 0 | PARTIAL_UPSTREAM | PORT | high |
...
### `capability:compression-session-boundary` — Compression session-boundary semantics

**Intent:** Prevent compressor-reported no-op results from creating a false session boundary.

**Status / disposition / confidence:** `PARTIAL_UPSTREAM` / `SPLIT` / `high`

**Historical issues/PRs:** none recorded
**Historical change records:** 0; **merge events:** 0

**Behavioral contract:**
- A context compressor reporting noop does not rotate or rewrite the session, even if it returns a newly allocated list.
- The current system prompt and session lock are released safely on the no-op path.

**Supporting historical commits / behavioral overlays:** 176646d2cd6c95fa49b9414f21ed9e781b0aaa84

**Current-dev survival:** Survives in agent/conversation_compression.py and its compression lifecycle tests.

**Upstream prior art/current implementation:** Frozen upstream has a semantic no-progress guard for equal compressor output, but the explicit status-reported-noop case is a separate residual contract.

**Unresolved questions:**
- Confirm the canonical status field for third-party context compressors before porting.

**Accounting source records:** behavior-overlay:commit:176646d2cd6c95fa49b9414f21ed9e781b0aaa84
...
**Overlay capability references:** capability:compression-session-boundary
...
### `capability:memory-trim-diagnostics` — Memory trim diagnostics

**Intent:** Attribute trim cost and memory pressure without making diagnostics a prerequisite for recovery.

**Status / disposition / confidence:** `PARTIAL_UPSTREAM` / `KEEP` / `medium`
...
**Current-dev survival:** Survives in hermes_cli/mem_trim.py through gc_ms/trim_ms, malloc_info fragmentation, VmSwap snapshots, and structured trim logs.

**Upstream prior art/current implementation:** Frozen upstream has basic RSS/RssAnon snapshots and trim logging, but not the fork's GC-versus-trim attribution, malloc_info fragmentation, or VmSwap diagnostics.

**Unresolved questions:**
- Which diagnostic fields are stable enough to expose as a long-term contract?
- Should malloc_info remain best-effort Linux/glibc-only instrumentation?

**Accounting source records:** commit:04f1af72be078cef69de538f1519f93a73088b0d

### `capability:memory-trim-policy` — Memory trim policy

**Intent:** Avoid expensive and low-value heap work while preserving cheap malloc_trim cadence.

**Status / disposition / confidence:** `PARTIAL_UPSTREAM` / `PORT` / `high`
...
- The 99.7% GC attribution explains the policy split; force behavior and invalid configuration retain safe fallbacks.

**Supporting historical commits / behavioral overlays:** 04f1af72be078cef69de538f1519f93a73088b0d

**Current-dev survival:** Survives in hermes_cli/mem_trim.py with threshold_mb, gc_cooldown_seconds, separate _last_gc_monotonic state, and independent malloc_trim cadence.

**Upstream prior art/current implementation:** Frozen upstream hermes_cli/mem_trim.py has a general cooldown around gc.collect()+malloc_trim, but no RSS low-water gate or independent GC cooldown; this is partial prior art, not equivalence.

**Unresolved questions:**
- Which upstream housekeeping callers should own the cheap trim cadence after porting?
- Should threshold_mb and gc_cooldown_seconds share the upstream config naming/default policy?

**Accounting source records:** commit:a9d2b9af4f800fef23fa7ecaf2ea270b43e326eb
...
### `capability:session-search-context-hydration` — Session search context hydration

**Intent:** Hydrate bounded surrounding context with a predictable lock scope while preserving result semantics.
...
**Behavioral contract:**
- Each search match receives only its bounded preceding, matching, and following message context.
- Context rows are fetched in bounded session batches and the position index is built outside the database lock.
- Decode failures degrade each match to an empty context rather than corrupting the result set.

**Supporting historical commits / behavioral overlays:** b879cbd332b8ac66ada6aaebfc5a9e61ae6cbe70

**Current-dev survival:** Survives in hermes_state.py session message search and context preview tests.

**Upstream prior art/current implementation:** Frozen upstream has per-match context hydration with equivalent -1/0/+1 semantics, but not the fork's bounded batch and lock-scope policy.

**Unresolved questions:**
- Confirm the current upstream search module seam and whether the batch size should remain 20.

**Accounting source records:** behavior-overlay:commit:b879cbd332b8ac66ada6aaebfc5a9e61ae6cbe70
...
### `capability:sqlite-write-contention-policy` — SQLite write contention backoff policy

**Intent:** Bound write contention with rollback-safe exponential backoff and a finite delay cap.
...
**Behavioral contract:**
- SQLite write contention retries roll back before retrying.
- Retry delay uses a 20ms base with jitter, exponential growth, and a 2s cap.
- Retry exhaustion and non-lock failures retain explicit error behavior.

**Supporting historical commits / behavioral overlays:** cc2531fbc6df8fdb34fca0b096b798eb53f970dd

**Current-dev survival:** Survives in hermes_state.py _execute_write and write-contention tests/logging.

**Upstream prior art/current implementation:** Frozen upstream has uniform 20-150ms retry jitter and slow-write diagnostics, not the fork's exponential 20ms-to-2s policy.

**Unresolved questions:**
- Revalidate attempt/deadline semantics when porting onto the current upstream writer.

**Accounting source records:** behavior-overlay:commit:cc2531fbc6df8fdb34fca0b096b798eb53f970dd
...
**Overlay capability references:** capability:sqlite-write-contention-policy
...
**Overlay capability references:** capability:session-search-context-hydration
