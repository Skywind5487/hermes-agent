...
def fixed3_shared_cross(db,K:int,B:int):
    b1=math.ceil(1.5*K); b2=math.ceil((1.5**2)*K)
    sql='''
    WITH RECURSIVE
    ...
    '''
    r=db.conn.execute(sql,{'b1':b1,'b12':b1+b2,'K':K,'B':B}).fetchone()
    roots=[] if not r['roots'] else r['roots'].split('|'); work=r['s1work']+r['s2work']+r['s3work']
    return {'roots':roots,'work':work,'bound_hit':(work>=B and len(roots)<K),'statements':1,'candidates':r['s1seeds']+r['s2seeds']+r['s3seeds']}
