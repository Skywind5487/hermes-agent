...
ALGORITHMS=[per_seed_point,pure_temp,fixed3_shared_cross]
            ...
            with self.subTest(scenario=scenario.name):
                ...
                try:
                    for fn in ALGORITHMS:
                        conn=sf.open()
                        try:
                            ref=reference(conn,scenario.k); result=fn(SimpleNamespace(conn=conn,observe_temp=False,after_snapshot_read=None),scenario.k,10_000)
                            self.assertEqual(result["roots"],ref,(scenario.name,fn.__name__,result,ref)); self.assertFalse(result["bound_hit"],(scenario.name,fn.__name__,result))
                        finally:conn.close()
                finally:sf.close()

    def test_small_c_candidates_are_unique_and_exact(self):
        for scenario in make_small_c_scenarios(values=(3,10,30)):
            self.assertEqual(len(scenario.candidates),len(set(scenario.candidates)),scenario.name); sf=ScenarioFile(scenario,filler=25)
            try:
                for fn in ALGORITHMS:
                    conn=sf.open()
                    try:
                        ref=reference(conn,scenario.k); result=fn(SimpleNamespace(conn=conn,observe_temp=False,after_snapshot_read=None),scenario.k,10_000); self.assertEqual(result["roots"],ref,(scenario.name,fn.__name__))
                    finally:conn.close()
            finally:sf.close()

    def test_global_budget_never_creates_fake_root(self):
        b=Builder(); chain=b.chain(100); scenario=Scenario("bound100",b.sessions,[chain[-1]],1,"bound fixture",False); sf=ScenarioFile(scenario,filler=0)
        try:
            for fn in ALGORITHMS:
                conn=sf.open()
                try:
                    result=fn(SimpleNamespace(conn=conn,observe_temp=False,after_snapshot_read=None),1,10); self.assertTrue(result["bound_hit"],fn.__name__); self.assertEqual(result["roots"],[],fn.__name__); self.assertLessEqual(result["work"],10,fn.__name__)
                finally:conn.close()
                conn=sf.open()
                try:
                    ref=reference(conn,1); result=fn(SimpleNamespace(conn=conn,observe_temp=False,after_snapshot_read=None),1,200); self.assertFalse(result["bound_hit"],fn.__name__); self.assertEqual(result["roots"],ref,fn.__name__)
                finally:conn.close()
        finally:sf.close()

    def test_fixed_plan_has_no_child_full_scan(self):
        result=eqp_probe(filler=1000); self.assertFalse(result["full_child_scan"],result["bad_details"]); self.assertFalse(result["suspicious_sessions_scan"],result["bad_details"])
...
class ProductionProfileSafetyTests(unittest.TestCase):
    ...
    def test_profiler_rejects_wrong_hash(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(SystemExit): profile_database(path,"0"*64,enforce_authoritative_counts=False)
