#!/usr/bin/env python3
"""Attribute resolver wall-time stalls to CPU, scheduler, faults, I/O, or swap.

Run the same command under TMPDIR=/tmp and TMPDIR=/dev/shm to compare disk-backed
and RAM-backed synthetic SQLite files without touching production state.db.
"""
...
def measured_call(fn, db, k, budget):
    ...
    wall_ms = delta(before, after, "wall_ns") / 1e6
    cpu_ms = delta(before, after, "cpu_ns") / 1e6
    return result, {
        "wall_ms": wall_ms,
        "cpu_ms": cpu_ms,
                ...
                try:
                    ...
                    for iteration in range(1, args.repeats + 1):
                        ...
                        print(
                            ...
                            f"noncpu={metrics['non_cpu_wall_ms']:.3f}ms majflt={metrics['major_faults']} "
                            f"read={metrics['read_bytes']} write={metrics['write_bytes']} "
                            f"steal={metrics['guest_steal_ms']:.3f}ms"
                        )
                        if args.cooldown_ms:
